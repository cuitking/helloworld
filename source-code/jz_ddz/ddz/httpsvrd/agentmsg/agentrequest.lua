...
function AgentRequest.callback(client_fd)
	msghelper:write_http_info("AgentRequest.callback: one connect "..client_fd)
	msghelper:check_session_timeout()
	...
	if not status then
		filelog.sys_error("AgentRequest.callback failed", client_fd, code)
		return
	end
	
	result = -1
	msghelper:write_http_info("AgentRequest.callback:", code, url, method, header, body)
	if code then
		if code ~= 200 then
		    ...
		    if channelinfo == nil then
		    	msghelper:write_http_info("AgentRequest.callback channelinfo == nil")
		    else
		    	...
		    	if result == 0 then
		    		...
		    		msghelper:write_http_info("AgentRequest.callback rechargeresponse:", rechargeresponse)
		...
		if url == sockethelper.socket_error then
			msghelper:write_http_info(string.format("AgentRequest.callback %s socket closed", url))
		else
			msghelper:write_http_info(string.format("AgentRequest.callback %s", url))
		end
	end
end
...
function AgentRequest.webclient(request)
	local channelinfo = msghelper:get_channel_byid(request.pay_type)

	if channelinfo == nil then
		filelog.sys_error(filename.." AgentRequest.webclient invalid pay_type", request)
...
function AgentRequest.versioninfo()
	local recvheader = {}
	local header = {
		["content-type"] = "application/x-www-form-urlencoded"
	}
...
function AgentRequest.generate_params(request, rechargeconf)
	local channelinfo = msghelper:get_channel_byid(request.pay_type)

	if channelinfo == nil then
		filelog.sys_error(filename.." AgentRequest.generate_params invalid pay_type", request, rechargeconf)
		base.skynet_retpack(nil)
		return
	end
